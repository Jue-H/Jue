...
const ProjectDetailsModal = ({ darkTheme, projectDetails }) => {
                ...
                {projectDetails?.sliderImages?.length > 0 &&
                  projectDetails?.sliderImages?.map((image, index) => (
                    <div className="item" key={index}>
                      <img className="img-fluid" alt={`Slide ${index + 1}`} src={process.env.PUBLIC_URL + "/" + image} />
                    </div>
                  ))}
              </Slider>
                  ...
                  <div className="col-md-5">
                    ...
                    {projectDetails?.projectInfo?.map((paragraph, index) => (
  ...
  {projectDetails?.url?.map((item, index) => (
    <span key={index}>
      <a href={item.link} target="_blank" rel="noopener noreferrer">
        {item.name}
      </a>
      {index < projectDetails.url.length - 1 && " | "} {/* Adds separator between links */}
    </span>
  ))}
</li>
                    </ul>
